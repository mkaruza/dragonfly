...
vector<ScoredMemberView> ZDiffOp(vector<OpResult<vector<ScoredMap>>>& maps, ShardId key_sid) {
  ...
  auto result = std::move(key_shard_map[0]);
...
void ZSetFamily::ZDiff(CmdArgList args, const CommandContext& cmd_cntx) {
  ...
  // Calculate diff set
  vector<ScoredMemberView> smvec = ZDiffOp(maps, sid);
...
void ZSetFamily::ZDiffStore(CmdArgList args, const CommandContext& cmd_cntx) {
  ...
  // Calculate diff between sets. We stil need to write 
  // destination key even it is empty set
  vector<ScoredMemberView> smvec = ZDiffOp(maps, sid);
